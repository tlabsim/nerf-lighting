...
def run_nerf(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
...
def render_rays(ray_batch,
    ...
    if N_importance > 0:
        rgb_map_0, disp_map_0, acc_map_0, mu_exp_0 = \
            rgb_map, disp_map, acc_map, mu_exp
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'mu_exp': mu_exp}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def render_nerf(H, W, focal,
    ...
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'mu_exp']
...
def create_nerf(args):
    ...
    model_nerf = init_nerf_model(
            D=args.netdepth_fine, W=args.netwidth_fine,
            input_ch=input_ch, output_ch=output_ch, skips=skips,
            input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)

    def network_query_nerf(inputs, viewdirs): return run_nerf(
        inputs, viewdirs, model_nerf,
        embed_fn=embed_fn,
        embeddirs_fn=embeddirs_fn,
        netchunk=args.netchunk)

    render_kwargs_train = {
        'network_query_fn': network_query_nerf,
        'perturb': args.perturb,
        'N_importance': 0,
        'network_fn': model_nerf,
        'network_fine': model_nerf,
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        ckpts = [os.path.join(basedir, expname, f) for f in sorted(os.listdir(os.path.join(basedir, expname))) if
                 ('model_fine_' in f and 'optimizer' not in f)]
    print('Found nerf ckpts', ckpts)
    if len(ckpts) > 0 and not args.no_reload:
        ft_weights = ckpts[-1]
        print('Reloading from', ft_weights)
        model_nerf.set_weights(np.load(ft_weights, allow_pickle=True))

    return render_kwargs_train, render_kwargs_test, model_nerf

def create_mu_model(args):
    ...
    def network_query_mu(inputs, viewdirs): return run_mu_model(
        inputs, viewdirs, model_mu,
        embed_fn=embed_fn,
        embeddirs_fn=embeddirs_fn,
        netchunk=args.netchunk)

    render_kwargs_train = {
        'network_query_fn': network_query_mu,
        'perturb': args.perturb,
        'network_fn': model_mu,
        'N_samples': args.N_samples,        
        'use_viewdirs': args.use_viewdirs,
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        ckpts = [os.path.join(basedir, expname, 'mu', f) for f in sorted(os.listdir(os.path.join(basedir, expname))) if
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, model_mu

def config_parser():

    import configargparse
    parser = configargparse.ArgumentParser()
    parser.add_argument('--nerf_config', is_config_file=True,
...
def train_mu(basedir):
    ...
    os.makedirs(os.path.join(basedir, expname, 'mu'), exist_ok=True)
    ...
    # Create nerf model
    nerf_render_kwargs_train, nerf_render_kwargs_test, model_nerf = create_nerf(args)   
    mu_render_kwargs_train, mu_render_kwargs_test, start, grad_vars, model_mu = create_mu_model(args)   
    ...
    models = {}
    models['model_nerf'] = model_nerf
    models['model_mu'] = model_mu    
    ...
    # Summary writers
    writer = tf.summary.create_file_writer(os.path.join(basedir, 'summaries', expname, 'mu'))
    ...
    for i in range(start, N_iters):
        ...
        with tf.GradientTape(persistent=False) as tape:
            ...
            rgb, disp, acc, mu_exp, extras = render(
                H, W, focal, chunk=args.chunk, rays=batch_rays,
                verbose=i < 10, retraw=True, **render_kwargs_train)
